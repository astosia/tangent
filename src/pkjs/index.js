...
//     // Refresh weather on launch if it's enabled, so the watch has fresh
//     // data before the next config save.
...
// // The watch periodically sends a dummy AppMessage (a single uint8 at key 0)
// // to ask the phone for a fresh weather reading - see tick_handler()'s
// // s_countdown logic in nomos.c. 
...
var isbw = (Pebble_platform === 'aplite' || Pebble_platform === 'diorite');
...
Pebble.addEventListener('webviewclosed', function(e) {
    ...
    applyTimezone(settings)
       ...
       applyTimezone(settings)
        ...
        .then(function(finalSettings) {
            // APIKEY_User is only needed here on the phone to call the
            // weather provider. Now that it's registered as a message key
            // (so Clay can key it properly instead of mangling it to
            // "NaN"), we have to explicitly drop it here so it's never
            // transmitted to the watch or printed in the sendAppMessage log.
            delete finalSettings[messageKeys.APIKEY_User];

            // Send every key on every save (rather than a computed delta).
            // This is slightly more bandwidth, but it means a save can never
            // silently fail to reach the watch for a subset of keys.
            sendAppMessage(finalSettings);
        });
    } 
});
